...
const getDateByMousePosition = (context: GanttContextProps, mouseX: number) => {
  const firstYear = context.timelineData.at(0)?.year;
  const timelineStartDate = new Date(firstYear ?? new Date().getFullYear(), 0, 1);
...
export const GanttFeatureRow: FC<GanttFeatureRowProps> = ({
  ...
  for (const feature of sortedFeatures) {
    ...
    while (subRow < subRowEndTimes.length) {
      const slotEnd = subRowEndTimes[subRow];
      if (slotEnd === undefined || slotEnd <= feature.startAt) {
        break;
      }
      subRow++;
    }

    const rowEndAt = feature.endAt ?? feature.startAt;

    // Update the end time for this sub-row
    if (subRow === subRowEndTimes.length) {
      subRowEndTimes.push(rowEndAt);
    } else {
      subRowEndTimes[subRow] = rowEndAt;
    }
